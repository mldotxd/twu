...
| 优先级 | 定义 | 说明 |
|--------|------|------|
| P1 | 核心功能正向用例 | 冒烟测试，必须全部通过 |
| P2 | 基本功能常用场景 + 核心功能反向用例 | 重要错误和边界值 |
| P3 | 基本功能不常用场景 | 低频但仍需覆盖的路径 |
| P4 | 基本功能反向用例 | 异常测试、边界值 |
| P5 | 不常用功能用例 | 可选，资源有限时跳过 |

> 判断逻辑：先看功能重要性（核心 > 基本 > 不常用），再看测试方向（正向 > 反向）。核心功能的反向用例（P2）优先级高于基本功能的正向用例。
